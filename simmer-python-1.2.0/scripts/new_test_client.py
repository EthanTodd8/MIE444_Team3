...
while s: 
    s = input('enter chr: ') 
    ser.write(s.encode()) # writes input string to Arduino thru COM port
    time.sleep(0.5) # you can also use pause(0.1) 
    print(ser.readline().strip().decode('ascii')) # reads from Arduino and prints in Python
...
while RUN_DEAD_RECKONING:
    ...
    if ct < len(CMD_SEQUENCE):
        ...
        if packet_tx:
            ...
            # Send emergency stop
            if float(responses[0][1]) <= 3: # if sensor reading is within 3 inches from wall
                print("Near front!")
                ...
                # Check adjacent sensors after stopped
                transmit(packetize('u3'))
                [responses_left, time_rx] = receive()
                print(f"Left sensor reading: {response_string('u3',responses_left)}")
                
                transmit(packetize('u1'))
                [responses_right, time_rx] = receive()
                print(f"Right sensor reading: {response_string('u1',responses_right)}")
                
                if float(responses_right[0][1]) > float(responses_left[0][1]): # if more space on right side
