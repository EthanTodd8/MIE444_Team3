...
def transmit(data: str):
    """Send a single ASCII character command safely."""
    try:
...
def read_line_safe(max_wait=1.0):
    """
    Read one line from the serial port with overall timeout.
    Returns decoded ASCII string or None on timeout.
    """
    start = time.time()
    buffer = b""
...
def read_us():
    """
    Request ultrasonic readings.
    Returns list: [Back, Left, Front, Right, BlockSensor]
    Retries on malformed data.
    """
    for _ in range(50):   # up to 50 retries
        transmit('u')
        time.sleep(0.02)  # allow Arduino to respond

        line = read_line_safe(max_wait=0.5)
        if not line:
            continue  # retry if no data

        parts = line.split(',')
        if len(parts) < 5:
            continue  # malformed line — ignore and retry

        try:
            values = [float(v) for v in parts[:5]]
            return values
        except ValueError:
            continue  # received bad numeric data

    raise TimeoutError("Ultrasonic sensor timeout: no valid readings after 50 attempts.")


def read_g(offset):
    """
    Request gyroscope reading.
    Returns list [angle] where angle is 0–360 adjusted for offset.
    Retries on invalid readings (0 or 180).
    """
    ...
    raise TimeoutError("Gyroscope timeout: no valid angle after 50 attempts.")
