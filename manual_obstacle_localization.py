...
PORT_SERIAL = '/dev/cu.KISI'    # Address identification, For mac use '/dev/cu.KISI', for Windows use 'COM7'
...
heading = 90 # 270° is down, 90 is up, 0 is left, 180 is right
...
while RUN:
    ...
    if cmd == 'u':
        readings = read_us() #readings in format [Back, Left, Front, Right, Down]
        ...
        if len(m_u) >= index + 5:
            current_readings = m_u[index:index+5]
